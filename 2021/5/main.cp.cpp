...
void testcase(__attribute__((unused)) signed tc){
    ...
    debug(n);
    int mx = 1001;
    vec<vec<int>> mp; init(mp, {mx, mx});
    rep(n) {
        char c; 
        int x1, y1; re(x1,c,y1);
        string s; re(s);
        int x2, y2; re(x2,c,y2);
        debug(x1,y1,x2,y2);
        if(x1 == x2) {
            for(int j = min(y1,y2); j <= max(y1,y2); j++) mp[x1][j]++;
        } else if(y1 == y2) {
            for(int i = min(x1,x2); i <= max(x1,x2); i++) mp[i][y1]++;
        } else if(x2 - x1 == y2 - y1) {
            if(x2 < x1) swap(x1,x2), swap(y1,y2);
            for(int d = 0; x1 + d <= x2; d++) mp[x1+d][y1+d]++;
        } else if(x2 - x1 == y1 - y2) {
            if(x2 < x1) swap(x1,x2), swap(y1,y2);
            for(int d = 0; x1 + d <= x2; d++) mp[x1+d][y1-d]++;
        }
    ...
    int res = 0;
    fon(i, mx) fon(j, mx) res += mp[i][j] >= 2;
    prl(res);
}
