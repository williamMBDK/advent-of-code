    ...
    fon(k, n) {
        ...
        if(type) {
            ...
            for(int i = a[0].fi; i <= a[0].se; i++) {
                ...
                ll ans = 0;
                set<int> ys = {a[1].fi, a[1].se, a[1].fi+1, a[1].se+1,a[1].fi-1,a[1].se-1};
                foe(p, starting[i]) {
                    ys.insert(p.fi);
                }
                foe(p, ending[i]) {
                ...
                foe(y, ys) {
                    ...
                    foe(e, starting[i][y]) {
                        active.insert({e.fi,e.se});
                    }
                    foe(e, ending[i][y]) {
                        assert(active.count({e.fi,e.se}));
                        active.erase(active.find({e.fi,e.se}));
                    }
                    map<int,int> cnt;
                    cnt[l] = 0;
                    cnt[r] = 0;
