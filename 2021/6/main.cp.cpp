...
vec<vec<int>> dp;
int solve(int am, int days) {
    if(dp[am][days] != -1) return dp[am][days];
    if(days == 0) return 1;
    if(am == 0) return dp[am][days] = solve(6, days - 1) + solve(8, days - 1);
    else return dp[am][days] = solve(am - 1, days - 1);
}
