...
// BEGIN code block of: data-structures/union-find
...
// END code block of: data-structures/union-find
void testcase(__attribute__((unused)) signed tc){
        ...
        fon(j, m) {
            ...
            foe(dir, dirs) {
                ...
                if(newi >= 0 && newj >= 0 && newi < n && newj < m) {
                    if(mp[i][j] == '9' || mp[newi][newj] == '9') continue;
                    if(mp[i][j] >= mp[newi][newj]) {
                        int idx2 = newi * m + newj;
                        debug(i,j,newi,newj,idx1, idx2);
                        dsu.union_sets(idx1, idx2);
                    }
                }
            }
        }
    ...
    vec<int> vals;
    foe(e, sets) vals.pb(e.se);
    sort(all(vals));
    int cnt = 0;
    for(auto e = vals.rbegin(); e != vals.rend(); e++) {
        cnt++;
        res *= *e;
        if(cnt == 3) break;
    }
